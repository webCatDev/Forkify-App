...
import { API_URL, DEV_API_KEY, RES_PER_PAGE } from './config';
...
export const loadRecipe = async function (id) {
  try {
    const data = await AJAX(`${API_URL}${id}?key=${DEV_API_KEY}`);
...
export const uploadRecipe = async function (newRecipe) {
    try{
        ...
        const data = await AJAX(`${API_URL}?key=${DEV_API_KEY}`,recipe)
